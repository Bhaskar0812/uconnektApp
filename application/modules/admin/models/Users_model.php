...
class Users_model extends CI_Model {
   ...
   //prepare post list query
    ...
    function getBusinessDetail($id){
        ...
        $this->db->select('um.address,um.bio,um.description,IF(um.company_logo = "" OR um.company_logo IS NULL,"'.$default_logo.'",concat("'.$logo.'",um.company_logo))as company_logo,aos.specializationName,job.jobTitleName,u.businessName,u.userId,u.email,u.phone,u.fullName,u.status,count(DISTINCT(f.favouriteId)) as favourites,count(DISTINCT(r.reviewId)) as reviews,count(DISTINCT(rec.recommendId)) as recommends,avg(r.rating) as rating,
            (case 
