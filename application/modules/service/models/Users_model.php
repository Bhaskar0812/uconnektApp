    ...
    function get_business_list($where){
        ...
        $defaultImg = base_url().DEFAULT_USER;
            ...
            if($userType->userType == 'business'){
                ...
                $this->db->select('u.userId,u.fullName,u.businessName,u.email,u.phone,u.userType,u.authToken,u.status,u.crd,u.deviceToken,u.deviceType,u.isProfile,u.isNotify,COALESCE(aos.specializationName,"") as specializationName,
...
function get_indivisual_search_list($search, $limit, $offset,$address,$city,$state,$country){
      ...
      //this query will work to add privious experience with same job title seperatly. to get result of user with privious work experience
       ...
       /* if(!isset($search['expectedSalaryFrom']) AND !empty($search['expectedSalaryTo']) AND $search['expectedSalaryTo'] != 99999999){
        ...
        $this->db->where(array('usr.status'=>1,'usr.userType'=>'individual','usr.isActive'=>1, 'usr.isVerified'=>1));
   ...
   function resend_mail($where,$data){
        ...
        if($res){
            $send['email'] = $email; 
            $base_url = substr(base_url(),0,-1);
            $send['link'] =  $base_url.'://home/verified_email/?token='.$data["verifiedLink"].'&email='.encoding($send['email']).'&id='.$where;
