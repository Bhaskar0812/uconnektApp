    ...
    function makeRequest_post(){ //make interview request..
        ...
        {       
            ...
            if($user_info->isNotify == 1){
               //pr($requestedFor);
            $notif_msg = $this->send_push_notification(array($registrationIds), $title, $body_send, $this->authData->userId, $notif_type,$userType,$profileImage);
             //pr($notif_msg);           
        ...
        else 
        {
          ...
          $dataAccept['upd']          = date('Y-m-d H:i:s',time());
            ...
            if($user_info->isNotify == 1 AND !empty($request_for)){
                ...
                $notif_msg = $this->send_push_notification(array($registrationIds), $title, $body_send,$request_for->request_for, $notif_type,$userType,$profileImage);
    ...
    function deleteRequest_post(){ //delete request
        ...
        if($res['type'] == "DR"){
            ...
            if($user_info->isNotify == 1 AND !empty($request_for)){
                ...
                $notif_msg = $this->send_push_notification(array($registrationIds), $title, $body_send, $request_for->request_by, $notif_type,$userType,$profileImage);
